...
Prior to the completion of the fine-grained TSAC sweep, sensitivity analysis used a provisional reference scenario called the stewardship-forward baseline (TSAC=5%, SOSAC=3%). This was a development placeholder used before balance points had been formally identified. The sweep confirmed that TSAC=5% is precisely the practical balance point — the setting that minimises the Gini coefficient while keeping Spearman rank correlation above 0.85. The stewardship-forward baseline was accordingly retired and replaced by the practical balance point, which has identical parameter values. Any outputs generated before this change that reference the stewardship-forward baseline are equivalent to outputs referencing the practical balance point.
