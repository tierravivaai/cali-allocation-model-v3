...
def test_gini_minimum_point_preset_turns_on_exclude_high_income():
    at = AppTest.from_file("src/app.py", default_timeout=30)
    at.run()

    at.checkbox(key="exclude_hi").set_value(False).run()
    balanced_button = [b for b in at.button if b.label == "5. Gini-minimum point"][0]
    balanced_button.click().run()

    assert at.checkbox(key="exclude_hi").value is True
    assert at.slider(key="tsac_beta_pct").value == 2.5
